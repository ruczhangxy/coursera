...
def lin_comb_mat_vec_mult(M, v):
    ...
    return sum(v[col] * col_dict[col] for col in col_dict)
...
def lin_comb_vec_mat_mult(v, M):
    ...
    return sum(v[row] * row_dict[row] for row in row_dict)
...
def Mv_mat_mat_mult(A, B):
    ...
    target_col_dict = {}
    for col, vec in col_dict.items():
        target_col_dict[col] = A * vec

    return coldict2mat(target_col_dict)
...
def vM_mat_mat_mult(A, B):
    ...
    target_row_dict = {}
    for row, vec in row_dict.items():
        target_row_dict[row] = vec * B
...
def dot_prod_mat_mat_mult(A, B):
    ...
    return Mat((A.D[0], B.D[1]), \
            {(x, y): (row_dict[x] * col_dict[y]) for x, y in product(A.D[0], B.D[1])})
