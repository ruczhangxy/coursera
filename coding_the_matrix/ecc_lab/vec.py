...
def add(u,v):
    ...
    assert u.D == v.D
    return Vec(u.D, {x:(u.f.get(x, 0) + v.f.get(x, 0)) for x in u.D})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum([u.f.get(x, 0) * v.f.get(x, 0) for x in u.D])

def scalar_mul(v, alpha):
    "Returns the scalar-vector product alpha times v"
    return Vec(v.D, {x:v.f.get(x, 0)*alpha for x in v.D})

def neg(v):
    "Returns the negation of a vector"
    return Vec(v.D, {x:v.f.get(x, 0)*-1 for x in v.D})
