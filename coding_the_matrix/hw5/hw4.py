...
def is_superfluous(L, i):
    ...
    v = L[i]
    L = L[0:i] + L[i+1:]
    sol = solve(coldict2mat(L), v)
    residual = v - coldict2mat(L) * sol
    if (residual * residual) > 10 **(-14):
        return False
    zero_vec = Vec(set(range(len(L))), {})
    return not sol == zero_vec
...
def superset_basis(S, L):
    ...
    '''
    for v in L:
        if not is_superfluous(S + [v], len(S)):
            S.append(v)
    return S
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    candidates = [v for v in S if v not in A]
    S.append(z)
