...
def morph(S, B):
    ...
    A = []
    ret = []
    candidates = [v  for v in S if v not in B]
    for z in B:
        w = exchange(candidates, A, z)
        A.append(z)
        candidates.remove(w)
        candidates.append(z)
        ret.append((z, w))
...
row_space_2 = [Vec({0,1,2,3},{0:1,1:4}),Vec({0,1,2,3},{1:1,2:1}),Vec({0,1,2,3},{2:1,3:1})]
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    cor = vec2rep(UV_basis, w)
    u = Vec(w.D, {})
    v = Vec(w.D, {})
    for i in sorted(cor.D):
        if i < len(U_basis):
            u += cor[i] * UV_basis[i]
        else:
            v += cor[i] * UV_basis[i]
...
def is_invertible(M): 
    ...
    cols = mat2coldict(M)
    rows = mat2rowdict(M)
    return (len(cols) == len(rows)) and rank(cols.values()) == len(cols)
